...
from dataverk_airflow import bucket_read, git_clone
...
def kubernetes_operator(
    ...
    **kwargs,
    ...
    if email:
        allowlist.append(
            os.getenv("AIRFLOW__SMTP__SMTP_HOST")
            + ":"
    ...
    on_finish_action = (
        OnFinishAction.DELETE_POD if delete_on_finish else OnFinishAction.KEEP_POD
    )
    ...
    if requirements_path:
        if use_uv_pip_install:
            cmds = [
                f"uv venv .local && . .local/bin/activate && uv pip install -r {POD_WORKSPACE_DIR}/{requirements_path} --no-cache-dir"
            ] + cmds
        else:
            cmds = [
                f"pip install -r {POD_WORKSPACE_DIR}/{requirements_path} --user --no-cache-dir"
            ] + cmds
...
def init_containers(
    is_composer: bool, repo: str, branch: str, run_as_user: str
) -> List[V1Container]:
    if is_composer:
        return [bucket_read(POD_WORKSPACE_DIR)]
    else:
        return [git_clone(repo, branch, POD_WORKSPACE_DIR, run_as_user)]
...
def volumes(is_composer: bool) -> List[V1Volume]:
    volumes = [V1Volume(name="dags-data")]
